...
- (UIViewController *)popCurrentView {
    ...
    if ([viewStack.lastObject isEqual:OutgoingCallViewController.compositeViewDescription]) {
        [viewStack removeLastObject];
    }
...
- (UIViewController *)_changeCurrentView:(UICompositeViewDescription *)view
	...
	if (![view equal:vc.currentView] || vc != self) {
		NSLog(@"Change current view to %@", view.name);
        // NSMutableArray *viewStack = [RootViewManager instance].viewDescriptionStack;
...
- (void)changeCurrentView:(UICompositeViewDescription *)view push:(BOOL)push {
    ...
    if (count > 0) {
        if (count == 1) {
            ...
            if (![view equal:[DialerView compositeViewDescription]] && ![view equal:[CallView compositeViewDescription]]) {
                [viewStack addObject:view];
            }else{
                // NSLog(@"Not add current view to stack!!!");
            }
        }
    }else{
        [viewStack addObject:view];
