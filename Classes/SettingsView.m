...
- (UITableViewCell *)newCellForIdentifier:(NSString *)identifier {
	...
	if ([identifier isEqualToString:kIASKPSToggleSwitchSpecifier]) {
        ...
        [((IASKSwitchEx *)cell.accessoryView) setOnTintColor: [UIColor colorWithRed:(1/255.0) green:(46/255.0) blue:(65/255.0) alpha:1.0]];
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated {
	...
	@try {
        ...
        //  Leo Kelvin
        /*
        NSString *settings = [[NSUserDefaults standardUserDefaults] objectForKey:@"firstTest"];
        if ([settings isEqualToString:@"0"]) {
		...
		NSLog(@"Failed to push view because it's already there: %@", e.reason);
...
- (void)appSettingChanged:(NSNotification *)notif {
	...
	if ([@"enable_video_preference" compare:notif.object] == NSOrderedSame) {
		...
		[[LinphoneAppDelegate sharedInstance] registerForNotifications:app];
	}
...
- (void)settingsViewControllerWillAppear:(IASKAppSettingsViewController *)sender
{
    if (sender.file == nil || [sender.file isEqualToString:@"Root"]) {
        ...
        _titleLabel.text = text_call_settings;
    }else{
...
- (void)setupFrameUIForView {
    ...
    if (SCREEN_WIDTH > 320) {
        ...
        [_titleLabel setFont:[UIFont fontWithName:HelveticaNeue size:20.0]];
    }else{
